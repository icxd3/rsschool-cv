# Hello, my name is El'ham

## Contacts
- GitHub: [icxd3](https://github.com/icxd3)
- Telegram: [@iCxd3](https://t.me/iCxd3)

## About
Junior front-end developer. Studying JavaScript and React at RS School
(Fullstack 2026 Q3) and Synergy University.

## Skills
- JavaScript, HTML5, CSS3, Markdown
- Git, GitHub, VS Code, Chrome DevTools
- React basics, Flexbox, CSS Grid, responsive layout

## Code examples
- [rsschool-cv](https://github.com/icxd3/rsschool-cv)

## Education
- Synergy University — Web Development (in progress)
- RS School — JS/Frontend Fullstack, 2026 Q3

## English
- A2, improving towards B1
